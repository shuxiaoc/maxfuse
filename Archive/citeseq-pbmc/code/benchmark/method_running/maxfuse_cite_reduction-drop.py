# script to produce viz umap maxfuse, drop antibody version (30 version)
import numpy as np
import pandas as pd
...
if not isExist:
    os.makedirs(out_dir)
# read in files
dropped_pro = pd.read_csv("/bench_test3/input/rank30.csv")
target = dropped_pro['target']
...
rna_active = rna_active[:, rna_active.std(axis=0) > 0.05] # remove static features double check
