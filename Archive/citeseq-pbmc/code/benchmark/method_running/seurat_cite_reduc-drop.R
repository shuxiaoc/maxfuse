...
library(matrixStats)
# read in files
out_root = "/home/bkzhu/super_mario/bench_test3/output/reduction-drop/"
...
# read
dropped_pro = read.csv("/home/bkzhu/super_mario/bench_test3/input/rank30.csv")
...
pre.anchors <- FindTransferAnchors(reference = x_obj, query = y_obj, 
                                   dims = 1:(max_comp-1), features = features) 
...
Int.anchors <- FindIntegrationAnchors(object.list = list_modality,
                                      dims = 1:(max_comp-1), anchor.features =features, k.filter = 10)
xy_int <- IntegrateData(anchorset = Int.anchors, dims = 1:(max_comp-1), k.weight = 10)
# 
