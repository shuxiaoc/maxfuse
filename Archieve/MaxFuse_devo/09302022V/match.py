...
class MaxFuse:
    ...
    def plot_canonical_correlations(
        ...
        if cca_components is None:
            cca_components = min([
                100,
                self.active_arr1.shape[1],
                self.active_arr2.shape[1],
                int(self.active_arr1.shape[0]//2),
                int(self.active_arr2.shape[0]//2)
            ])

        # if cca_components >= 1000:
        #     cca_components = min([100, self.active_arr1.shape[1], self.active_arr2.shape[1]])
        #     warnings.warn(f'cca_components>=1000, reset it to {cca_components}.')
    ...
    def propagate(
            self, wt1=0.7, wt2=0.7, svd_components1=None, svd_components2=None, randomized_svd=False, svd_runs=1, verbose=True
    ):
        """
    ...
    def get_matching(self, order=None, target='pivot'):
        ...
        # return_format: 'dict' or 'list'
        #     If 'dict', then return a defaultdict(list) of structure
        #     {row: [(col1, val1), (col2, val2), ...]} where the matched pairs are (row, col1), (row, col2), ...
        #     and the similarity scores are val1, val2, ...
        #     If 'list', then return a list of length three. Say (rows, cols, vals).
        #     Each component is a list, and the matched pairs are (rows[i], cols[i]),
        #     and their similarity scores are vals[i]
