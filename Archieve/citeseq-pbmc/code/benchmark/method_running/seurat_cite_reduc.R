...
### temp
#set.seed(42)
#ranid = sample(25000, 10000)
###

# read
rna = readMM(paste0(in_dir,"rna.txt"))#[ranid,]
protein = read.csv(paste0(in_dir,"pro.csv"))#[ranid,]
meta = read.csv(paste0(in_dir,"meta.csv"))#[ranid,]
...
Int.anchors <- FindIntegrationAnchors(object.list = list_modality,
                                      dims = 1:30, anchor.features =features, k.filter = 10)
xy_int <- IntegrateData(anchorset = Int.anchors, dims = 1:30, k.weight = 10)
# 
