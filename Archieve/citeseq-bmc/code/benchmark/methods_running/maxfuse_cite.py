...
if not isExist:
    os.makedirs(out_dir)
# read in files
rna = mmread(in_dir + "rna200.txt")
protein = pd.read_csv(in_dir + "pro200.csv")
meta = pd.read_csv(in_dir + "meta200.csv")
rna_names = pd.read_csv("/bench_test4/input/rna_names.csv")
#
# start processing
protein = protein.drop('Unnamed: 0', axis=1)
annotationlv1 = meta['lv2_bk'].to_numpy()
#
rna_adata = ad.AnnData(
    rna.tocsr(), dtype=np.float32
)
rna_adata.var_names = rna_names['names']
rna_adata.obs_names = meta['name']
...
mask = ((rna_shared.std(axis=0) > 0.1) & (protein_shared.std(axis=0) > 0.1)).A1
